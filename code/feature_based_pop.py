...
class Trainer(object):
    def __init__(self, config):
        ...
        self.config = config
        file_path = os.path.abspath(__file__)
        self.log_path = os.path.dirname(file_path) + '/../log/feature_based_pop_log_{c}.txt'.format(c=config['city'])
    ...
    def load_model(self):
        model = FeatureBasedPopulationPrediction(config=self.config).to('cuda')
        return model
