...
class FT_Dataset_POP(Dataset):
    def __init__(self, config=None, dataset_type=None):
        ...
        self.region_coord_path = self.root_path + 'data/pop_{c}_region_coord_{dt}'.format(c=config['city'], dt=dataset_type)
        self.poi_sortby_zorder_path = self.root_path + 'data/pop_{c}_poi_sort_by_zorder_{dt}'.format(c=config['city'], dt=dataset_type)
