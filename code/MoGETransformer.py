...
class MoGESelfAttention(nn.Module):
...
class MoGESelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MoGEIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MoGELayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, expert_selection, split_idx):
        ...
        if expert_selection == 'p_and_s':
            ...
            mlp_output = torch.cat([poi_mlp_output, img_mlp_output], dim=1)
...
class MoGEEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([MoGELayer(config=config, is_ps_expert=(i >= self.ps_layer_start_idx)) for i in range(config['num_hidden_layers'])])
