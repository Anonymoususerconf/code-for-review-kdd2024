...
import argparse
from utils import * 
from ft_dataset_cap import *
from models import FinetuneCommercialActivenessPrediction
...
class Trainer(object):
    def __init__(self, config):
        ...
        self.log_path = os.path.dirname(file_path) + '/../log/finetune_cap_log_{c}.txt'.format(c=config['city'])
        self.pretrain_state_path = os.path.dirname(file_path) + '/../checkpoint/'
        

    def get_dataloader(self):
        train_dataset = FT_Dataset_CAP(config=self.config, dataset_type='train')
        val_dataset = FT_Dataset_CAP(config=self.config, dataset_type='val')
        test_dataset = FT_Dataset_CAP(config=self.config, dataset_type='test')
    ...
    def load_model(self):
        model = FinetuneCommercialActivenessPrediction(config=self.config).to('cuda')
