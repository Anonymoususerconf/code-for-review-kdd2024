...
class FT_Dataset_CAP(Dataset):
    def __init__(self, config=None, dataset_type=None):
        ...
        self.region_coord_path = self.root_path + 'data/cap_{c}_region_coord_{dt}'.format(c=config['city'], dt=dataset_type)
        self.poi_sortby_zorder_path = self.root_path + 'data/cap_{c}_poi_sort_by_zorder_{dt}'.format(c=config['city'], dt=dataset_type)
        ...
        with open(self.id_of_region_path, 'r') as f:
            for line in f:
                ...
                self.id_of_region.append(line[1])
